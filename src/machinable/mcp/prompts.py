...
def register_prompts(mcp: Any, ctx: MCPContext) -> None:
    ...
    def investigate(hypothesis: str) -> str:
        ...
        return (
            ...
            "2. LOCATE/SCAFFOLD: `list_modules`; if nothing lays out the sweep, "
            "`write_source` it (encode the experiment arms as `version_*` methods → "
            "`~versions`, and a flat fan-out over inputs/seeds as an `axis_*` "
            "staticmethod → `~~axis`; reach for an aggregate interface with its own "
            "`launch()` only when the sweep needs its own config, an order, or prior "
            "results). Fix any import error the write returns. Get human approval.\n"
