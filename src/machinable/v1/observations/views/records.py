...
class Records(RecordCollection):
    ...
    def refresh(self, disable_sql=False):
        """Reloads records from the write
        ...
        if (
            isinstance(self._items, FileNotFoundError)
            or self._items is FileNotFoundError
        ):
            if self._scope != "default":
                raise FileNotFoundError(
                    f"Records for scope '{self._scope}' could not be found"
                )
            self._items = []
            return
        ...
        if isinstance(self._items, list):
            for r, row in enumerate(self._items):
                data = {
                    k: cast(k, v) for k, v in row.items() if k in self.indexed_columns
                }
                data.update({"id": r})
    ...
    def info(self, echo=False):
        ...
        info = {"columns": columns, "indexed_columns": self.indexed_columns}
