...
class Dmosopt(Interface):
    class Config(BaseModel):
        ...
        def valid_optimization_settings(cls, params: dict) -> dict:
            _t = {
                ...
                "space": dict[str, tuple[int | float, int | float]] | None,
                "population_size": int,
                "num_generations": int,
                "resample_fraction": float,
                "distance_metric": Callable | Literal["crowding", "euclidean"],
                "n_epochs": int,
                "save_eval": bool,
                "file_path": str | None,
                "save": bool,
                ...
                "optimizer_name": Literal["nsga2", "age", "smpso", "cmaes"] | str,
                ...
                "termination_conditions": bool | dict | None,
                #
                "di_crossover": Any,  #
                "di_mutation": Any,  #
            }
            ...
            for path, alias, kw in [
                ...
                if isinstance(target := payload.get(path, None), str):
                    ...
                    try:
                        ...
                        raise ValueError(
                            f"Could not resolve import path '{target}'"
                            f" for '{path}': {_ex}"
                        ) from _ex
                    ...
                    if (d := payload.get(kw, None)) is not None:
                        ...
                        if any(
                            ...
                            # if function accepts keyword arguments,
                            # we cannot validate :-(
                            continue
                        ...
                        for key in d.keys():
                            if key not in sig.parameters:
                                ...
                                raise ValueError(
                                    f"Invalid {kw} for {target}. Found"
                                    f" `{key}`, but signature is {message[:-2]}"
                                )
    ...
    def __call__(self) -> None:
        if MPI.COMM_WORLD.Get_rank() == 0:
            ...
            if "local_random" not in params and "random_seed" not in params:
                params["random_seed"] = self.execution.seed
            for f in [
                "feature_dtypes",
                "objective_names",
                "constraint_names",
    ...
    def get_best(
        ...
        if region is None:
            if len(data["epochs"]) > 5000:
                # optimize for speed since best solutions will be found
                # in the last epochs
    ...
    def num_evals_total(self) -> int:
        # n_epochs - 1 since epoch 0 is using the initial sampling,
        # so there are no additional evals
