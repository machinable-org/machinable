...
class Globus(Storage):
    ...
    def commit(self, interface: "Interface") -> str:
        try:
            ...
            # if os.path.normpath(src) != os.path.normpath(
            #     self.local_path(interface.uuid)
            # ):
            ...
            if (
                ...
                raise RuntimeError(
                    f"You do not have the right permissions."
                    f" Try removing {self.config.auth_filepath}"
                    " and authenticating again with the appropriate"
                    " identity provider."
                ) from e
            raise e
    ...
    def retrieve(self, uuid: str, local_directory: str, timeout: int = 5 * 60) -> bool:
