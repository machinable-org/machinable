        ...
        for executable in self.pending_executables:
            ...
            if job := Job.find_by_name(executable.id):
                if job.status in ["PENDING", "RUNNING"]:
                    print(
                        f"{executable.id} is already launched"
                        f" with job_id={job.job_id}, skipping ..."
                    )
                    continue
            ...
            script += self.dispatch_code(
                executable, project_directory=source_code, python=python
            )
            ...
            try:
                ...
                job_id = False
            print(
                f"{job_id}  named `{resources['--job-name']}`"
                f" for {executable.local_directory()}"
                f" (output at {resources['--output']})"
            )
